...
;                         Default is to keep original input spacings.
...
;                - fixed correction for off-limb scaling when projecting
;
...
;   yp[find]=-999999.
  ...
  temp=interp2d(float(pmap.data),xp,yp,xr,yr,_extra=extra,missing=missing)  
...
;-- make sure all map tags are ok
 ...
 if do_drot then rtime=anytim2utc(new_time,/vms)
