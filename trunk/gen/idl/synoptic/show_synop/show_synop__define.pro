...
self->setprop,org='day',ext='',ldir=user_synop_data,err=err,up_prep=1b,$
  last_time=1b,plotman=have_proc('plotman__define')
...
;-- check if this selection already cached
   ...
   if status then begin
    if have_method(data,'has_data') then begin
     ...
     if ~status then message,'Object is missing data.',/cont
    endif
    if clobber then status=0b
   endif
...
;-- check if sending to Prepserver
   ...
   endif else message,'Reusing object from last read.',/cont
...
;   was last saved in PLOTMAN
 ...
 clobber=self->getprop(/clobber)
 replace=0b & nodup=1b
 if read_again or clobber then begin
  replace=1b & nodup=0b
 endif
...
sock_fits,file,header=header,/nodata,err=err
...
;-- sub-interval selection
 ...
 else: message,'No option selected',/cont
endcase
