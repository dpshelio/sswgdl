...
;
;     http://msslxr.mssl.ucl.ac.uk:8080/eiswiki/attach/DataProAnalysis/spec_gauss_widget_tutorial.pdf
;
; INPUTS
;
    ...
    CASE event.value OF
     ...
     ; =============
      2: BEGIN
       ...
       ; 
        parinfo=replicate({fixed:0, limited:[0,0], limits:[0.D,0.D]},n_elements(init))
       ...
       ;
        ...
        background.y1=aa[naa-1]
        background.sigy1=sigmaa[naa-1]
        IF nback EQ 2 THEN BEGIN 
          background.y0=aa[naa-2]
          background.sigy0=sigmaa[naa-2]
        ENDIF ELSE BEGIN
...
IF n_elements(yrange) EQ 0 THEN BEGIN
  i=where(yy NE miss2,ni)
  yrange=[min(yy[i]),max(yy[i])*1.1]
ENDIF 
