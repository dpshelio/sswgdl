...
;
if keyword_set(carrington) then lon_name = 'crln_obs' else $
  lon_name = 'hgln_obs'
index = tag_index(pos, lon_name)
if index ge 0 then phi = pos.(index)*dtor else phi = 0.d0
cos_phi = cos(phi)
sin_phi = sin(phi)
...
;
if keyword_set(carrington) then lat_name = 'crlt_obs' else $
  lat_name = 'hglt_obs'
index = tag_index(pos, lat_name)
if index ge 0 then theta = pos.(index)*dtor else theta = 0.d0
cos_theta = cos(theta)
sin_theta = sin(theta)
