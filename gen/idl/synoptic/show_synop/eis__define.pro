...
rhost_back='umbra.nascom.nasa.gov'
...
if count eq 0 then message,'No EIS files found.',/cont
...
;-- prep level-0 data
 ...
 if edata->getfitslev() eq 0 then self->prep,edata,_extra=extra else $
  message,'Data already prepped.',/cont
...
;-- extract wavelengths 
 ...
 if verbose then message,'Reading '+trim(nwaves)+' wavelengths...',/cont
...
if is_string(err) then begin
 message,'Could not read header - '+file,/cont
 return,valid
endif
...
valid=(count0 gt 0)
if ~valid then begin
 message,'Not a valid EIS file - '+file,/cont
 return,valid
endif
