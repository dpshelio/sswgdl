...
common have_cal,compiled
err=''
if ~exist(compiled) then compiled=0b
if compiled then return,1b
...
if ~have_cal and ~compiled then begin
 ...
 if self->have_path() then begin
  ...
  if network then begin
   if verbose then message,'Will attempt to download calibration files from remote server...',/cont
...
for i=0,nfiles-1 do begin
 ...
 if (level eq 1) then begin
  self->fits::read,ofile[i],_extra=extra
  j=self->get(/count)
  continue
 endif
 ...
 for k=0,nimg-1 do begin
  ...
  if (image_no[k] gt -1) and (image_no[k] lt nsub) then begin
   if have_path then begin
    ...
    if ~have_path then xack,err1,/suppress
    if ~have_cal then xack,err2,/suppress
    message,'Skipping prep.',/cont
...
valid=count ne 0
if ~valid then begin
 message,'Invalid EIT file - '+file,/cont
 return,valid
endif
...
if level eq 0 then begin
 ...
 if verbose and (level eq 1) then message,'EIT image is already prepped.',/cont
endif
...
; message,'Cannot degrid non-rectangular image',/cont
;endif
