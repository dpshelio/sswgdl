...
if self->valid_bktimes() or (*self.b0user)[0] ne -1 or (*self.b1user)[0] ne -1 then begin
  ...
  if self->valid_bktimes() then begin
    ...
    nbk = sizbk.dimensions[1] > 1
  	...
  	if total(ybk) eq 0. then message,'No data in background time intervals.', /cont
...
if ybk[0] eq -1 then message, 'No valid background times or user background levels are defined.', /cont
...
case type of
  ...
  'Background' : begin
    ...
    endelse
    if not exist(value) then err_msg = 'Background time intervals should be an array of start/ends, or -1 for none.'
    end
  ...
  'Integration' : begin
    if n_elements(times) eq 2 then value = anytim2tai(times) else begin
      if times[0] eq -1 or times[0] eq 0 then value = -1
    endelse
    if not exist(value) then err_msg = 'Integration time interval should be a single start,end, or -1 to disable.'
    end
 ...
 for iarch = 0,narch do begin
   ...
   if sdac_used then begin
      ...
      rd_goes,times,data,trange=[dstart,dend],_extra=extra,err=err,$
        type=type,gdata=gdata,sat=sat_num, verbose=verbose
   endelse
...
out=['GOES 91 -  01-Jul-1974   to   17-Oct-1975', $
     ...
     'GOES 13 -  not operational yet', $
     'GOES 14 -  01-Dec-2009   to   present' ]
...
if not self->valid_bktimes() then return
...
; 
function goes::get_gev, timestart, timeend, count=count, struct=struct, show=show, _extra=_extra

checkvar, timestart, anytim2utc(self->getprop(/tstart),/vms)
checkvar, timeend, anytim2utc(self->getprop(/tend),/vms)

gev = get_gev(timestart, timeend, count=count, err=err)
...
if count eq 0 then begin
   message, /cont, 'No events found in time range 
   return,-1
endif
