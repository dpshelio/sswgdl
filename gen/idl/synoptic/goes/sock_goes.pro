...
;   data = [data, temporary(data0)]
  ...
  if (get_nbytes(data(0)) gt 4) then begin
      ...
      if ss[0] eq -1 then $
        message,'Error reading file ' + infil + ' No data or data is all zero. Will keep searching...', /info
    ...
    if (status eq 0) then status = 2  ;dont change status if set by "no file found" error
      ...
      if goesdig then begin
         if max([data.lo, data.hi]) eq 0. then begin
           message, 'Data in file ' + infil + ' is all zero. Will keep searching...', /info
           qdone=0
           status=2
         endif else begin
           qdone = 1
           status = 0
           if keyword_set(verbose) then message,'Data successfully read from file ' + infil, /info
         endelse
      endif else begin
         qdone = 1
         state = 0
