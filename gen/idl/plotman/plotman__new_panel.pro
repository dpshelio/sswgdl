...
; if number set then returns number of panel instead of pointer to panel structure.  If all is
; set and number is set, then returns all panel numbers that match description passed in.
; (all has no effect if number is not set.)

function plotman::desc2panel, panel_desc, number=number, all=all, latest=latest
...
pro plotman::new_panel, description, description=desc, $
  ...
  if keyword_set(nodup) then begin
    panel = self -> desc2panel(description, /number)
